...
public class ExtendedRIRBuilder implements IRelationBuilder {
	...
	public IRelation buildRelationFrom(IProject project) {
		...
		this.initAttrMapGhosts(project.getGhostTypes());
		for (IType type : this.definedTypes) {
			this.addTypeAttributesToRelation(type);
		}
		for (IType type : this.definedTypes) {
			if (this.normalAttrMap.get(type) == null) {
				continue;
			}
	...
	private void propagateAttrDownward(IType type, Attribute attr) {
		...
		for (IType subType : type.getAllSubtypes()) {
			...
			if (normalAttrSet == null) { // Some types can have 0 attributes
				continue;
			}
	...
	private void addExtendedAttributeToRelation(IType type, IMethod method) {
		...
		if (isRoot) { // Check if there is a Ghost that implements the same method
			for (IType superType : superTypes) {
				if (superType.isGhost() && this.extendedAttrMap.get(superType) != null) {
					for (Attribute ghostAttr : this.extendedAttrMap.get(superType)) {
						if (ghostAttr.getName().equals(signature)) {
							// the attribute has to be added to the map of type, or else it won't be propagated
							this.addToAttrMap(type, ghostAttr, extendedAttrMap);
							this.extendedRelation.addRelation(type, ghostAttr);
						}
					}
				}
			}
	...
	private static Set<String> getObjectMethods() { // mostly generated by llm
        ...
        for (Method m : cls.getDeclaredMethods()) {
            ...
            String exceptions = Arrays.stream(m.getExceptionTypes())
                    .map(Class::getTypeName)
                    .collect(Collectors.joining(", "));
            ...
            objectMethods.add(signature);
        }
        return objectMethods;
	}
	
	private void initAttrMapGhosts(Collection<IType> ghostTypes) {
		for (IType ghost : ghostTypes) {
			this.putLocalInterface(ghost);
			for (String signature : this.localInterfaces.get(ghost)) {
				final Attribute extendedGhostAttr = new Attribute(signature, true, false, true, ghost);
				this.addToAttrMap(ghost, extendedGhostAttr, extendedAttrMap);
			}
		}
	}
